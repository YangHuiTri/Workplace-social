	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="40">ID</th>
				<th width="150">评论文章</th>
				<th width="40">文章id</th>
				<th width="80">文章作者</th>
				<th width="80">被评人</th>
				<th width="100">被评内容</th>
				<th width="80">评论人</th>
				<th width="80">评论人头像</th>
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td>{{$val->article_author}}</td>
				<td>{{$val->bei_author}}</td>
				<td>{!!$val->bei_content!!}</td>
				<td>{{$val->author_name}}</td>
				<td><img width="60" src="{{$val->author_avatar}}" class="image"></td>
				<td>{{$val->content}}</td>
				<td>{{$val->created_at}}</td>
				<td class="td-manage">
					<a title="删除" href="javascript:;" onclick="comment_del(this,'<?php echo $val->id; ?>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
				</td>
			</tr>
			@endforeach
		</tbody>
...
$(function(){
	$('.table-sort').dataTable({
		...
		"aoColumnDefs": [
		  ...
		  {"orderable":false,"aTargets":[0,2,4,5,6,7,9,11]}// 制定列不参与排序
		]
	});
